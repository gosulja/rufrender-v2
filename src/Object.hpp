...
class Object {
public:
    Object(const std::vector<float>& vertices, const std::vector<unsigned int>& indices, const char* diffusePath, const char* specularPath)
        : m_position(0.0f), m_rotation(0.0f), m_scale(1.0f), m_vbo(Buffer::Type::VERTEX), m_ebo(Buffer::Type::INDEX) {
        setupMesh(vertices, indices);
        m_diffuseTexture = std::make_unique<Texture>(diffusePath, true);
        m_specularTexture = std::make_unique<Texture>(specularPath, true);
    }
    ...
    void Draw(Shader* shader, float dt) {
        shader->use();
        m_diffuseTexture->bind(0); // bind diffuse texture to texture unit 0
        m_specularTexture->bind(1); // bind specular texture to texture unit 1
